# Hi 👋, I'm Nitesh Kumar

### QA Engineer | AI Testing | SDET | Playwright | TypeScript

QA professional with **7+ years of QA experience**, including **4.2 years in Software Testing**, with strong experience in **Manual Testing, AI Testing, Web & Mobile Testing, API Testing and Test Automation**.

My primary focus is on **AI Quality Engineering and AI Testing**, including **Prompt Testing, LLM Evaluation, RAG Testing, Hallucination Detection, AI Agent Testing and AI Response Validation**.

I also specialize in building scalable test automation solutions using **Playwright, TypeScript and JavaScript**, with experience across functional, regression, integration and end-to-end testing.


---

## 🛠️ Skills & Tech Stack

### 🤖 AI Testing & Quality Engineering

- 🧠 AI Testing
- 🎯 AI Quality Engineering
- 💬 LLM Testing
- 📊 LLM Evaluation
- ✍️ Prompt Testing
- 🧩 Prompt Engineering
- 🔎 RAG Testing
- ⚠️ Hallucination Detection
- 🤖 AI Agent Testing
- ✅ AI Response Validation
- 🎯 Context Adherence Testing
- 🔍 Response Relevance Testing
- 🧮 Semantic Correctness
- 🛡️ Refusal Accuracy
- 🔐 Prompt Injection Testing

### 🧪 Quality Assurance & Manual Testing

- 🔍 Manual Testing
- ⚙️ Functional Testing
- 🔄 Regression Testing
- 🚦 Smoke Testing
- 🩺 Sanity Testing
- 🧭 Exploratory Testing
- 🔗 Integration Testing
- 🔄 End-to-End Testing
- 👥 User Acceptance Testing (UAT)
- 🌐 Web Testing
- 📱 Mobile Testing
- 🌍 Cross-Browser Testing
- ♿ Accessibility Testing
- 📐 Responsive Testing
- 🚀 Release Validation
- 🐞 Defect Management
- 🔬 Root Cause Analysis

### 🎭 Test Automation

- 🎭 Playwright
- 📘 TypeScript
- 🟨 JavaScript
- 🏗️ Page Object Model (POM)
- 🧱 Automation Framework Design
- 🔄 Automated Regression Testing
- 🔗 End-to-End Automation

### 🔌 API & Database Testing

- 🔌 REST API Testing
- 🚀 Postman
- 🗄️ SQL
- 🔗 API Integration Testing
- 🧪 API Functional Testing

### ⚙️ Tools & Technologies

- 🐙 Git
- 🐙 GitHub
- 📋 Jira
- 🌐 Chrome DevTools
- 📱 BrowserStack
- 🔥 Firebase
- 🎨 Figma

### 🔄 QA Process & Methodologies

- 🔄 Agile / Scrum
- 📐 SDLC
- 🧪 STLC
- 📝 Test Planning
- 🎯 Test Strategy
- 🧩 Test Case Design
- 🐞 Defect Lifecycle
- 📊 Risk-Based Testing
- 🚀 Release Validation
- 🔍 Production Verification
- 🔬 Root Cause Analysis

---

## 💻 Technology Stack

### 🎭 Automation & Programming

<p>
  <img src="https://img.shields.io/badge/Playwright-45ba4b?style=for-the-badge&logo=playwright&logoColor=white" />
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white" />
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black" />
</p>

### 🔌 API & Testing Tools

<p>
  <img src="https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white" />
  <img src="https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white" />
  <img src="https://img.shields.io/badge/Chrome%20DevTools-4285F4?style=for-the-badge&logo=googlechrome&logoColor=white" />
  <img src="https://img.shields.io/badge/BrowserStack-FF6C37?style=for-the-badge&logo=browserstack&logoColor=white" />
</p>

### 🛠️ Development & Collaboration

<p>
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
  <img src="https://img.shields.io/badge/Jira-0052CC?style=for-the-badge&logo=jira&logoColor=white" />
</p>

### 🤖 AI & Quality Engineering

<p>
  <img src="https://img.shields.io/badge/AI%20Testing-412991?style=for-the-badge" />
  <img src="https://img.shields.io/badge/LLM%20Testing-8A2BE2?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Prompt%20Testing-FF69B4?style=for-the-badge" />
  <img src="https://img.shields.io/badge/RAG%20Testing-6A5ACD?style=for-the-badge" />
</p>
